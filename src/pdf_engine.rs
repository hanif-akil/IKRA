...
impl PdfEngine {
    ...
    pub fn search_text(&self, query: &str) -> Vec<usize> {
        self.search_text_with_count(query).0
    }

    /// Returns (pages_with_match, total_match_count_across_all_pages).
    pub fn search_text_with_count(&self, query: &str) -> (Vec<usize>, usize) {
        let mut pages   = Vec::new();
        let mut total   = 0usize;
        if query.trim().is_empty() { return (pages, total); }
        let q = query.to_lowercase();
        if let Some(doc) = &self.doc {
            for (i, page) in doc.pages().iter().enumerate() {
                if let Ok(text) = page.text() {
                    let body = text.all().to_lowercase();
                    // Count non-overlapping occurrences on this page
                    let mut count = 0usize;
                    let mut start = 0;
                    while let Some(pos) = body[start..].find(&q) {
                        count += 1;
